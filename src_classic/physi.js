window.Physijs = (function() {
	...
	Eventable.prototype.dispatchEvent = function( event_name ) {
		...
		if ( this._eventListeners && this._eventListeners.hasOwnProperty( event_name ) ) {
	...
	Physijs.Scene = function( params ) {
		...
		this._worker = new Worker( Physijs.scripts.worker || 'physijs_worker.js', { type: 'classic' } );
		this._worker.transferableMessage = this._worker.webkitPostMessage || this._worker.postMessage;
                this._worker.onerror = e => {
                     console.log("Worker: Error event handler is fired.",e);
                };
		...
		params = params || {};
		params.ammo = Physijs.scripts.ammo || 'ammo.min.js';
	...
	Physijs.Scene.prototype._updateCollisions = function( data ) {
		...
		for ( id1 in this._objects ) {
			...
			if ( collisions[ id1 ] ) {
			    ...
			    if(object._physijs && object._physijs.touches){ object._physijs.touches.length = 0; }
	...
	Physijs.Scene.prototype.add = function( object ) {
		...
		if (! object._physijs ) {
			...
			if ( object instanceof Physijs.Vehicle ) {
			    //THREE.Mesh.prototype.add.call( this, object.mesh );
	...
	Physijs.Scene.prototype.remove = function( object ) {
		...
		if ( object instanceof Physijs.Vehicle ) {
			...
			if ( object._physijs !== undefined && object._physijs.id !== undefined ) {
	...
	Physijs.Mesh.prototype.setCollisionFlags = function( flag ) {
	    this._physijs.collision_flags = flag;
	}

	Physijs.Mesh.prototype.physijs_clone = function(){
	    let geometry = this.geometry;
	    let mesh = this.clone.call( this );
	    mesh._physijs = this._physijs;
	...
	Physijs.HeightfieldMesh = function ( geometry, material, mass, xdiv, ydiv) {
	    ...
	    //if(! this._physijs){ this._physijs = {}; } 
	    if(! geometry){ geometry = this.geometry; } 
	    ...
	    this._physijs.xpts = (typeof xdiv === 'undefined') ? Math.sqrt(count) : xdiv + 1;
	    this._physijs.ypts = (typeof ydiv === 'undefined') ? Math.sqrt(count) : ydiv + 1;
		...
		for ( var i = 0; i < count; i++ ) {
		    ...
		    index = a + ( ( this._physijs.ypts - b - 1 ) * this._physijs.ypts );
		    if (geometry instanceof THREE.BufferGeometry) {
			points[i] = geometry.attributes.position.getZ( index );
		    }else{
			points[i] = geometry.vertices[ index ].z;
		    }
	...
	Physijs.HeightfieldMesh.prototype.constructor = Physijs.HeightfieldMesh;
        Physijs.HeightfieldMesh.prototype.sync = function(){
	    var a, b, index;
		...
		index = a + ( ( this._physijs.ypts - b - 1 ) * this._physijs.ypts );
		if (geometry instanceof THREE.BufferGeometry) {
		    this._physijs.points[i] = geometry.attributes.position.getZ( index );
		}else{
		    this._physijs.points[i] = geometry.vertices[index].z;
		}
	    }
	...
	Physijs.ConcaveMesh = function( geometry, material, mass ) {
		...
		for ( i = 0; i < geometry.faces.length; i++ ) {
			...
			if ( face instanceof THREE.Face3) {
			    ...
			    if (geometry instanceof THREE.BufferGeometry) {
				triangles.push([
					{ x: vertices.getX(face.a), y: vertices.getY(face.a), z: vertices.getZ(face.a) },
					{ x: vertices.getX(face.b), y: vertices.getY(face.b), z: vertices.getZ(face.b) },
					{ x: vertices.getX(face.c), y: vertices.getY(face.c), z: vertices.getZ(face.c) }
				]);
			    }else{
				triangles.push([
				...
				triangles.push([
					{ x: vertices.getX(face.a), y: vertices.getY(face.a), z: vertices.getZ(face.a) },
					{ x: vertices.getX(face.b), y: vertices.getY(face.b), z: vertices.getZ(face.b) },
					{ x: vertices.getX(face.d), y: vertices.getY(face.d), z: vertices.getZ(face.d) }
				]);
				triangles.push([
					{ x: vertices.getX(face.b), y: vertices.getY(face.b), z: vertices.getZ(face.b) },
					{ x: vertices.getX(face.c), y: vertices.getY(face.c), z: vertices.getZ(face.c) },
	...
	// Physijs.Vehicle
	Physijs.Vehicle = function( mesh, tuning ) { //scene, 
                //this.world = scene;
		...
		this.addWheelMesh( wheel, connection_point, wheel_direction, wheel_axle, suspension_rest_length, wheel_radius, is_front_wheel, tuning );
	};
	Physijs.Vehicle.prototype.addWheelMesh = function( wheel, connection_point, wheel_direction, wheel_axle, suspension_rest_length, wheel_radius, is_front_wheel, tuning ) {
	...
	Physijs.Vehicle.prototype.resetSuspension = function() {
	    if ( this.world ) {
		this.world.execute( 'resetSuspension', {
		    id: this._physijs.id
		} );
	    }
	}
