...
//import("../js-module/ammo.babylon.wasm.js").then(function(Ammo) {
//	importScripts( "../js-module/ammo.min.js" );
//	importScripts( "../js-module/ammo.babylon2.wasm.js" );

//import('../js-module/ammo.babylon.wasm.js')
//  .then(function(Ammo){
...
if(! _transform){//XXXXX HACK
	...
	switch ( description.type ) {
		...
		case 'heightfield':
			...
			//example:
			//const ptr: number = Ammo._malloc(4 * mesh.nbPointsX * mesh.nbPointsY * 4)
			//mesh.values.forEach((value, index) => {
			//    this.ammo.HEAPF32[ptr + (index * 4)] = value
			//})

			//ORG var ptr = Ammo.allocate(4 * description.xpts * description.ypts, "float", Ammo.ALLOC_NORMAL);
...
try{
    ...
    console.log("physijs_worker: CreateShape Error catch:", e, "description=", description );
    return;
}
    if (!shape) return
...
try{
//console.log(description.materialId, _transform);		//DEBUG XXXXXXXXXXX
	motionState = new Ammo.btDefaultMotionState( _transform ); // #TODO: btDefaultMotionState supports center of mass offset as second argument - implement
//console.log(description.materialId, "motionState", motionState);	//DEBUG	 XXXXXXXXXXX
	rbInfo = new Ammo.btRigidBodyConstructionInfo( description.mass, motionState, shape, _vec3_1 );
}catch(e){
    console.log("physijs_worker:385 Error catch:",e, _transform, description.mass, motionState);
    return;
}
...
public_functions.addVehicle = function( description ) {
	...
	//_objects[ description.rigidBody ].setCollisionFlags( 2 ); //CF_KINEMATIC_OBJECT (additional setting)
...
//if(details.force>0){console.log("3",_vehicles, details.force,details.id,_vehicles[details.id]);}
	if ( _vehicles[details.id] !== undefined ) {
...
try{
    ...
    console.log("physijs_worker:removeObject Error catch:",e, details.id,_objects[details.id]);
    return;
}
...
public_functions.resetSuspension = function( details ) {
    let vehicle = _vehicles[ details.id ];
